...
class Message extends Component {
	...
	render() {
		...
		const history_messages = t_hstry.map((msg) => {
			if (
				msg.from === this.props.loggedInUser &&
				msg.to === this.props.reciepient
			) {
				if (msg.type === "audio") {
					return (
						<Card
							key={msg.timestamp}
							className="img-from-client "
							style={{ border: "none" }}
						>
							<ReactAudioPlayer src={msg.enc} controls />
				...
				return (
					<Card
						key={msg.timestamp}
						className="msg-from-client col-offset-left-8 col-4"
					>
				...
				msg.from === this.props.reciepient &&
				msg.to === this.props.loggedInUser
			) {
				if (msg.type === "audio") {
					return (
						<Card
							key={msg.timestamp}
							className="img-to-client "
							style={{ border: "none" }}
						>
							{/* <img class="image" src={msg.enc} alt="video" /> */}
							<ReactAudioPlayer src={msg.enc} controls />
				...
				return (
					<Card
						key={msg.timestamp}
						className="msg-to-client col-offset-8 col-4"
					>
						<CardBody className="cardbody">
							{" "}
							<MessageText text={msg.text} />{" "}
